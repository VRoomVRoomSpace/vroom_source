...
def turnthefuckeroff(command):
    payload = {'access_token':'c2f6dac1645d068c313efdc7cbf425f2e97066c2','params':command}
    r = requests.post("https://api.spark.io/v1/devices/51ff6f065082554910260887/car", data=payload)
    print(r.text)
...
if __name__ == '__main__':
    ...
    #ans = raw_input("Do you want to use the keyboard? Answer Y or N.\n")
      
    isManual = True;
    while isManual:
        choice = raw_input('dir?')
        if choice == 'w':
            turnthefuckeroff("go")
        elif choice == 's':
            turnthefuckeroff("verse")
        elif choice == 'd':
            turnthefuckeroff("right")
        elif choice == 'a':
            turnthefuckeroff("left")
        elif choice == ' ':
            turnthefuckeroff("stop")
        elif choice == 'q':
            turnthefuckeroff("stop")
            isManual = False;
